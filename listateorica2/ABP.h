    ...
    if((*raiz)->info == elem){
        ...
        if((*raiz)->esq == NULL && (*raiz)->dir == NULL){
            ...
            *raiz = (*raiz)->esq;
            ...
            //Estrategia 3.1:
            NO* Filho = (*raiz)->dir;
            while(Filho->esq != NULL)//Localiza o MAIOR valor da subarvore esquerda
                Filho = Filho->esq;
...
int procuraMaior(ABP *raiz){
    if((*raiz)->dir == NULL) return (*raiz)->info;
    procuraMaior(&(*raiz)->dir);

}

int maiorElemento(ABP *raiz){
    if(raiz == NULL) return 0;
    printf("O maior elemento eh %d\n", procuraMaior(raiz));
    return procuraMaior(raiz);
}

int procuraMenor(ABP *raiz){
    if((*raiz)->esq == NULL) return (*raiz)->info;
    procuraMenor(&(*raiz)->esq);

}

int menorElemento(ABP *raiz){
    if(raiz == NULL) return 0;
    printf("O menor elemento eh %d\n", procuraMenor(raiz));
    return procuraMenor(raiz);
}
